...
{
   aluno *vet_aluno = new (nothrow) aluno[*n + 1];

   if(vet_aluno == 0) {
      cout << "Erro de alocação de memoria, por favor tente novamente.";
      return 0;
   }
...
{
   // declara um novo aluno recebendo sua referencia 
   aluno *vetAlunoRemovido;
   /*pecorrer e verificar se matricula desse elemento no vetor de aluno
   e igual a matricula recebida no parametro.*/
   for (int i = 0; i < *n; i++)
   {
      if(v[i].matricula == matricula) {
         v[i] = v[*n - 1];
      
      vetAlunoRemovido = new (nothrow) aluno[*n - 1];

      if(vetAlunoRemovido == 0) {
         cout << "Erro de alocação, por favor tente novamente";
         return 0;
      }

      for (int i = 0; i < *n - 1; i++)
      {
         vetAlunoRemovido[i] = v[i];
      }
      (*n)--;
      delete [] v;

      return vetAlunoRemovido;   
      }
   }
}

//*vetor = valor da variavel apontada
//vetor = endereço armazenado no ponteiro
